...
{
    public static partial class Accessory_States
    {
        [Serializable]
        [MessagePackObject]
        public class CoordinateData
        {
            ...
            public void AccStateSyncconvert(int outfitnum, out List<Support.AccStateSync.TriggerProperty> TriggerProperty, out List<Support.AccStateSync.TriggerGroup> TriggerGroup)
            {
                TriggerProperty = new List<Support.AccStateSync.TriggerProperty>();
                TriggerGroup = new List<Support.AccStateSync.TriggerGroup>();
                ...
                foreach (var slotinfo in Slotinfo)
                {
                    ...
                    for (var state = 0; state < maxstate; state++) //default conversion
                    {
                        ...
                        TriggerProperty.Add(new Support.AccStateSync.TriggerProperty(outfitnum, slotinfo.Key, binding, state, visible, priority));
                    }
                    ...
                    if (shoetype != 2 && !shoebinding) //binded to indoor or outdoor state only make false priorty to not show
                    {
                        ...
                        {
                            TriggerProperty.Add(new Support.AccStateSync.TriggerProperty(outfitnum, slotinfo.Key, othershoe, state, false, 3));
                        }
                    }
                    ...
                    if (shoebinding && shoetype == 2) //binded to a shoe, but other shoe needs same work
                    {
                        ...
                        for (var state = 0; state < maxstate; state++)
                        {
                            ...
                            TriggerProperty.Add(new Support.AccStateSync.TriggerProperty(outfitnum, slotinfo.Key, othershoe, state, visible, priority));
                        }
                    }
                ...
                foreach (var NameData in Names)
                {
                    ...
                    TriggerGroup.Add(new Support.AccStateSync.TriggerGroup(outfitnum, NameData.Key, NameData.Value.Name) { States = states.OrderBy(x => x.Key).ToDictionary(x => x.Key, x => x.Value) });
                }
            }

            public void AccStateSyncconvert(List<Support.AccStateSync.TriggerProperty> TriggerProperty, List<Support.AccStateSync.TriggerGroup> TriggerGroup)
            {
                var triglistbyslot = new List<List<Support.AccStateSync.TriggerProperty>>();
            ...
            private List<Support.AccStateSync.TriggerProperty> ClothingNotConversion(int outfitnum, int slot, List<int[]> statelist, int clothingkind)
            {
                var TriggerProperty = new List<Support.AccStateSync.TriggerProperty>();
                bool prioritycheck;
                for (var state = 0; state < 4; state++)
                {
                    prioritycheck = state % 3 == 0;
                    TriggerProperty.Add(new Support.AccStateSync.TriggerProperty(outfitnum, slot, clothingkind, state, (prioritycheck) ? ShowClothNotState(state, statelist) : false, (prioritycheck) ? 2 : 0));
                }
